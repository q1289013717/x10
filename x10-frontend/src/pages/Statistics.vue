            ...
            <table class="w-full">
              ...
              <tbody>
                <tr v-for="day in dailyData" :key="day.date" class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td class="px-4 py-4 text-right"><span :class="['font-medium', getRateColor(day) > 0 && getRateColor(day) === 'emerald' ? 'text-emerald-600' : getRateColor(day) === 'blue' ? 'text-blue-600' : 'text-slate-700']">{{ formatMoney(day.completed) }}</span></td>
