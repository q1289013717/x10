            ...
            <div class="p-5">
              <div class="flex items-center justify-between mb-4">
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-2xl font-bold text-slate-900 mt-2 truncate">¥{{ (stats.todayTarget / 10000).toFixed(1) }}万</p>
                </div>
              ...
              <div class="mt-2">
                <div class="flex items-center justify-between mb-2">
                  ...
                  <span class="text-xs font-medium text-blue-600">{{ todayPercent }}%</span>
                </div>
                <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div class="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-500" :style="{ width: `${todayPercent}%` }" />
                </div>
                <p class="text-xs text-slate-500 mt-2 truncate">已完成 ¥{{ (stats.todayCompleted / 10000).toFixed(1) }}万</p>
              </div>
            </div>
          </div>
...
onMounted(async () => {
  ...
  try {
    ...
    await taskStore.fetchTasksByRange(startStr, end)
